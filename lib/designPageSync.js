...
const DESIGN_FIELD_DEFS = [
  ...
  // cta_url / reviews_url : type single_line_text_field (PAS 'url') — la définition
  // existante côté Shopify est déjà en single_line_text_field (créée ainsi à l'origine),
  // et le type d'une définition de metafield est immuable. Un désalignement ici fait
  // échouer metafieldsSet avec "Type 'url' must be consistent with the definition's
  // type: 'single_line_text_field'" pour CHAQUE page ayant déjà ces champs (bug
  // corrigé le 2026-08-17 : la synchro échouait sur les 16 pages design existantes).
  // De plus deepLink()/reviewsUrl() renvoient une URL absolue, ce qu'un champ 'url'
  // typé refuserait de toute façon si jamais la query string contenait certains
  // caractères — single_line_text_field est aussi le choix le plus permissif ici.
  { key: 'cta_url', name: 'RX Design — Lien de personnalisation', type: 'single_line_text_field' },
  { key: 'reviews_url', name: 'RX Design — Lien avis produit', type: 'single_line_text_field' },
  { key: 'alt_products', name: 'RX Design — Produits alternatifs', type: 'json' },
  // Variantes par marché (design réadapté) — définitions déjà créées manuellement
  // dans l'admin Shopify ; on les redéclare ici juste pour que ensureDesignFieldDefinitions()
  // reste la source de vérité complète (idempotent, tolère "déjà existant").
  { key: 'image_url_en', name: 'RX Design — Image (EN)', type: 'url' },
  { key: 'cta_url_en', name: 'RX Design — Lien de personnalisation (EN)', type: 'single_line_text_field' },
  { key: 'image_url_es', name: 'RX Design — Image (ES)', type: 'url' },
  { key: 'cta_url_es', name: 'RX Design — Lien de personnalisation (ES)', type: 'single_line_text_field' },
];
...
// ---------------------------------------------------------------------------
// Disponibilité par marché (EN/ES) — le FR n'a pas d'entrée dans item.markets,
// c'est toujours les champs racine (customizationId/imageUrl) qui font foi.
// Résolution : available=false -> design absent sur ce marché (rien à écrire) ;
// available=true && needsAdaptation=false -> on réutilise les champs FR (le
// thème retombe dessus côté Liquid, on n'écrit rien de plus ici) ;
// available=true && needsAdaptation=true -> on écrit image_url_<marché> et
// cta_url_<marché> à partir des valeurs spécifiques au marché. Le variantId,
// lui, ne change jamais par marché (même produit/variante, seul le
// customization-id Teeinblue diffère).
// ---------------------------------------------------------------------------
const MARKET_CODES = ['en', 'es'];

function marketMetafields(item) {
  const markets = item.markets || {};
  const fields = [];
  MARKET_CODES.forEach(code => {
    const m = markets[code];
    if (!m || !m.available || !m.needsAdaptation) return;
    if (m.imageUrl) {
      fields.push({ namespace: DESIGN_NAMESPACE, key: `image_url_${code}`, type: 'url', value: m.imageUrl });
    }
    if (m.customizationId) {
      fields.push({
        namespace: DESIGN_NAMESPACE,
        key: `cta_url_${code}`,
        type: 'single_line_text_field',
        value: deepLink(item.productHandle, item.variantId, m.customizationId),
      });
    }
  });
  return fields;
...
function buildPageFields(item, productsByHandle) {
  ...
  const metafields = [
    ...
    { namespace: DESIGN_NAMESPACE, key: 'cta_url', type: 'single_line_text_field', value: deepLink(item.productHandle, item.variantId, item.customizationId) },
    { namespace: DESIGN_NAMESPACE, key: 'reviews_url', type: 'single_line_text_field', value: item.productHandle ? reviewsUrl(item.productHandle) : '' },
    ...
    // Champs spécifiques marché (EN/ES) — uniquement si le design a été explicitement
    // marqué comme réadapté pour ce marché (item.markets.<code>.needsAdaptation).
    // On ne touche JAMAIS aux traductions natives Shopify (Translate & Adapt) des
    // champs texte ci-dessus : ceux-ci restent toujours écrits en FR uniquement.
    ...marketMetafields(item),
...
async function syncDesignPages() {
  ...
  for (const item of items) {
    try {
      // Éligibilité : respecte un choix explicite (case cochée/décochée dans le
      // back-office). Si l'item n'a jamais été touché (créé avant cette
      // fonctionnalité, ou importé automatiquement depuis une commande), on ne
      // génère une page QUE pour les designs officiels/curatés (source ===
      // "official") — les captures brutes de commandes clients partagent
      // souvent un titre générique identique entre elles (ex. "T-SHIRT RUNNING
      // HOMME À PERSONNALISER" répété), ce qui produit du contenu quasi-dupliqué
      // et de mauvaises pages SEO si on leur crée une page à chacune.
      const wantsPage = item.createPage === true
        ? true
        : item.createPage === false
          ? false
      ...
      if (item.pageId) {
        ...
        // Plusieurs designs peuvent partager le même titre (variantes d'un même
        // libellé produit) -> même handle généré -> collision. On retente une
        // fois avec un suffixe tiré du customizationId (stable et unique par
        // design) plutôt que d'abandonner.
        const baseHandle = `design-${handleize(item.title)}`;
        ...
        for (let attempt = 0; attempt < 2 && !page; attempt++) {
          ...
          if (errors?.length) {
            const handleTaken = errors.some(e => /handle.*taken/i.test(e.message));
            if (handleTaken && attempt === 0 && item.customizationId) {
              handle = `${baseHandle}-${item.customizationId.slice(0, 8)}`;
              continue;
            }
